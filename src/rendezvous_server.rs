    ...
    async fn handle_tcp(
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            match msg_in.union {
                ...
                Some(rendezvous_message::Union::RegisterPk(rk)) => {
                    // Previously unconditionally NOT_SUPPORT: RegisterPk was only ever
                    // implemented in handle_udp. Clients using TCP/WS transport (e.g. to
...
#[inline]
fn build_rk_res(res: register_pk_response::Result) -> RendezvousMessage {
    let mut msg_out = RendezvousMessage::new();
    msg_out.set_register_pk_response(RegisterPkResponse {
        result: res.into(),
        ..Default::default()
